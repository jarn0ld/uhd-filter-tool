...
int main(int argc, char** argv)
{
    ...
    desc.add_options()
        ...
        ("rate", po::value<double>()->default_value(32.0e6), "[rate] master clock rate")
        ("mask", po::value<std::string>()->default_value("rx_frontends/A/"), "[mask] mask to use for filter search")
        ("show", po::value<std::string>(), "[path] show filter details");
    ...
    usrp->set_master_clock_rate(rate);
		...
		std::cout << std::endl << "Found the following filters matching \""
			<< search_mask <<"\": " << std::endl;
...
void write_rx_fir_i16(uhd::usrp::multi_usrp::sptr usrp,
		...
		uhd::digital_filter_fir<boost::int16_t>::sptr filter, std::string path)
{
...
void write_back_rx_filter(uhd::usrp::multi_usrp::sptr usrp,
  uhd::filter_info_base::sptr filter, std::string path)
{
